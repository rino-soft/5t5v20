...
function dialog_nueva_facventa(div_dialog, direccion)
{
    burl = $('#b_url').val();
    this.cargar_contenido_html(div_dialog, direccion, 0);//modulo=0 no sirve
    $("#" + div_dialog).dialog({
        ...
        buttons: [
            ...
            },
            {
                ...
                disabled: true,
                click: function () {
                    imp_factura_venta($('#id_ov_pf').val());
            ...
            },
            {
                ...
                click: function () {
                    ...
                    ttrab = "";
                    if ($("#tipotrabajo #cano").is(':checked'))
                        ttrab += "*Canon";
                    if ($("#tipotrabajo #ecan").is(':checked'))
                        ttrab += "*Extra Canon";
                    if ($("#tipotrabajo #inst").is(':checked'))
                        ttrab += "*Instalacion";
                    if ($("#tipotrabajo #insu").is(':checked'))
                        ttrab += "*Insumos";
                    if ($("#tipotrabajo #vent").is(':checked'))
                        ttrab += "*Venta";
                    if ($("#tipotrabajo #otro").is(':checked'))
                        ttrab += "*Otro";

                    //alert(ttrab);
                    if (validacion == 1) {
                        //alert(data_des + ", " + data_cant + " ," + data_pu + " ," + data_sub);
                        $.post(burl + "factura_venta/factura_venta_save", {
                            ...
                            'comentario_penalidad': $("#comentario_penalidad").val(),
                            //'tipo_factura': $("#tipo_trabajo :selected").val(),
                            'tipo_factura': ttrab,
                            'fecha_prov_pago': $("#fec_estimada_cobro").val(),
                            'comentarios_seguimiento': $("#comentario_seguimiento_nuevo").val(),
                            'comentarios_seguimiento_old': $("#coment_old_seguiento").val()
                        }
                        , function (data) {
...
function add_fila_factura_venta() {
...
function del_fila_factura_venta(variable) {
...
function search_and_list_factura_venta(div_resultado)
{
    burl = $('#b_url').val();

    $("#" + div_resultado).html('<div class="cargando_circulo" ></div><div></div><div class="f20 alin_cen">Cargando...</div>');
    $.post(burl + "factura_venta/busqueda_lista_factura_venta", {
...
function imp_factura_venta(id_fac)
{
    // alert (registro);
    //alert($("#burl").val());
    aleatorio = Math.round(Math.random() * (100 - 1) + 1);
    baseurl = $("#b_url").val() + 'impresiones_pdf/factura_venta_impresion/' + id_fac;
    // alert(baseurl);
    miven = window.open(baseurl, aleatorio + id_fac, "menubar=0,location=1,status=1,scrollbars=0, width=800,height=600");
//alert("ok");
}
function imp_factura_ventacontinuoPapel(id_fac)
{
    // alert (registro);
    //alert($("#burl").val());
    aleatorio = Math.round(Math.random() * (100 - 1) + 1);
    baseurl = $("#b_url").val() + 'impresiones_pdf/impr_fac_conti/' + id_fac;
    // alert(baseurl);
    miven = window.open(baseurl, aleatorio + id_fac, "menubar=0,location=1,status=1,scrollbars=0, width=800,height=600");
//alert("ok");
}
...
function search_factura_venta(event)
{
