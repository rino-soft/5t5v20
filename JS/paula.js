    ...
    if($("#check"+id_prod).is(':checked'))
    {
      ...
      //  alert($("#"+id_prod+" #res").val());
        if($("#"+id_prod+" #res").val()==1)
            {
                //seriales_obtenidos_max("+id_prod+","+cont+",'Ingreso',"+alm+");</script>
            respuesta=" <div id='serial_old"+id_prod+"-"+cont+"'  style='margin-top:15px'></div><script>seriales_registradas_in("+id_prod+","+cont+",'Ingreso',"+alm+");</script>";
            }
        //alert("res"+respuesta);
        ...
        codigo="<div class='grid_20 fondo_amarillo bordeado ' id='sel"+id_prod+"-"+cont+"' >\n\
                   ...
                   <div class='grid_5'>\n\
                        <div class='grid_2 espizq10'><input title='cantidad' id='cant' value='1' onkeyup='validarSiNumero(\"cant\")' type='text' class='grid_2 alin_cen margin_cero input_redond_20' placeholder='Cantidad'></input></div>\n\
                    ...
                    <div class='grid_8'>"+respuesta+"\n\
                        <div id='otra_opcion"+id_prod+"-"+cont+"' class='ocultar'>\n\
                            <div class='grid_5'>\n\
                                <div class='grid_2 espizq10'><input title='codigo propio' id='cp'  onkeyup='$(this).val($(this).val().substring(0,9).toUpperCase());' type='text' class='grid_2 alin_cen margin_cero input_redond_20' placeholder='Codigo Propio' placeholder='Otorgado por el Sistema' disabled='disabled' ></div>\n\
...
//    vec=cadena.split("* SN: ");
//    
//    $("#sel"+div_padre+" #"+serial).val(vec[1]);
//    $("#sel"+div_padre+" #"+codprod).val(vec[0]);
//    
//}
function asigna_serial_codprop(origen,serial,codprod,div_padre)
{
  ...
  //  alert(cadena);
    vec=cadena.split("* SN: ");
    //alert("---vec1--- "+vec[1]+"---vec2---"+vec[0]);
    $("#sel"+div_padre+" #"+serial).val(vec[1]);
    sn=vec[1];
    cp=vec[0].substring(4);
    $("#sel"+div_padre+" #"+codprod).val(vec[0].substring(4));
    //alert('ingrera al post');
    $.post(burl+"movimiento_almacen/obtener_comentario_articulo_serie",{
        'cp':cp,
        'sn':sn        
    }
    ,function(data){
        //alert(data);
        $("#sel"+div_padre+" #coment").html($.trim(data));
        cad=div_padre.split("-");
        mostrar_quitar_coment(cad[0],cad[1],1);
...
//{ //alert('entra'+div_resultado+","+id_pro+","+op+","+cant);
            ...
            click: function() {
               ...
               // genera_sol_mat("c_op",burl+"solicitud_material/codigo_ope_sol_mat/"+id_sm+"/"+id_ma); 
                ...
                ,function(data){
                   ...
                   setTimeout(function(){
                       busca_lista_mi_sol_mat_enviada('lista_solicitud_material');
...
function seleccionar_menu_no_sirve_anterior_funcion(id_sp){
...
function seleccionar_menu(id_sp,campo,tipo)//tipo = menu, tipo = control {
//alert(id_sp+":"+$("#c"+id_sp).is(':checked'));
{
    t='co';
    if(tipo=="menu"){
    ...
    if($("#"+t+id_sp).is(':checked'))
    
    {
        ids=$("#"+campo).val();
        vec=ids.split(",");   
        $("#"+campo).val($("#"+campo).val()+','+$("#"+t+id_sp).val());
        burl=$('#b_url').val();
    ...
    {
        ids=$("#"+campo).val();
        $("#"+campo).val($("#"+campo).val().replace(','+$("#"+t+id_sp).val(),''))    
        //$("#controles"+id_sp).html("");      
    }
...
{
    $('#cssmenuu > ul > li:has(ul)').addClass("has-sub");
    $('#cssmenuu > ul > li > a').click(function() {

        var checkElement = $(this).next();

        $('#cssmenuu li').removeClass('active');
        ...
        if((checkElement.is('ul')) && (!checkElement.is(':visible'))) {
            ...
            $('#cssmenuu ul ul:visible').slideUp('normal');
     ...
     /*   open: function(event, ui) {
       ...
       */
        buttons: {
            ...
            "Guardar": function() {
                var expedido= $('#exp :selected').val();
                var genero= $('#gen :selected').val();
                $.post(burl+"usuario/guardar_usuario",{
                    'id_user':$('#id_user').val(),
                    'nom':$('#nom').val(),
                    'app':$('#app').val(),
