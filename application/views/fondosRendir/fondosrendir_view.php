...
<div class="row tile_count">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12  ">
        <div class="col-md-9 col-sm-9 col-xs-12 f20 negrilla ">
            Solicitudes de Fondos a Rendir
        </div>
...
<div class="row col-md-12 col-sm-12 col-xs-12">
    <div class="btn btn-primary " style="float: left; display: table-cell " 
         onclick="mostrar_fomrulario_sol_fr(0);" >
        Nuevo SOlicitud de Fondos a Rendir
    </div>
</div>
...
<div id="lista_ordenCompra"class="f10 row">
    <div class="col-md-12 col-sm-12 col-xs-12 ">
        <div class="x_panel">
            <div class="x_title">
                <span class="f16 negrilla">Lista de Solicitudes de Fondos a Rendir</span>   
                ...
                <table id="datatable" data-order='[[0 , "desc" ]]' class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody class ="f12">
                        ...
                        foreach ($registros->result() as $reg) {
                            ...
                            <tr>
                                <td><?php echo $reg->id_sol_frendir; ?></td>
                                <td><a class='btn btn-success btn-xs bordeado filas' onclick="mostrar_fomrulario_sol_fr('<?= $reg->id_sol_frendir ?>')">editar</a>
                                    <a class='btn btn-primary btn-xs ' href='<?= base_url() . "control_proyecto/reg_ordecompra/" . $padre . "/" . $reg->id_sol_frendir; ?>'>Print</a>
                                ...
                                <td title="<?=$reg->titulo?>"><?php echo substr($reg->titulo, 0, 50); //$reg->nombre;        ?></td>
                                <td><?php echo $reg->user_nom; //$reg->nombre;        ?></td>
                                <td><?php echo $reg->fh_registro; //$reg->nombre;        ?></td>
                                 <td class="negrilla f16 alin_der">

                                    <table class="" cellspacing="0" border="0" style="width: 100%" >
                                        <?php
                                        foreach ($proy_montos[$reg->id_sol_frendir]->result() as $proy_m) {

                                            echo "<tr class='filas'><td class='f12 ' title='".$proy_m->nombre."'>" .  substr($proy_m->nombre, 0, 20). "</td><td class='f14 negrilla alin_der '>" . number_format($proy_m->monto_proyecto, 2, ",", ".") . "</td></tr>";
                                        }
                                        ?>
                                    </table>
                                ...
                                <td class="negrilla f16 alin_der"><?php echo number_format($reg->monto_rendido, 2, ",", "."); ?></td>
                               

                                <td><?php echo $reg->user_reg; //$reg->nompreproy;        ?></td>
