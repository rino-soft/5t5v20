...
if ($b == "_")
    $b = 'Ninguno';
$this->phpexcel->getActiveSheet()->setCellValue('A2', 'Periodo: del ' . $d . ' al ' . $h . ' , Terminos de busqueda: ' . $b);
...
$this->phpexcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D' . $punt, 'NUMERO DE AUTORIZACION')
        ->setCellValue('E' . $punt, 'ESTADO')
        ->setCellValue('F' . $punt, 'NIT CLIENTE')
        ->setCellValue('G' . $punt, 'NOMBRE O RAZON SOCIAL')
        ->setCellValue('H' . $punt, 'IMPORTE TOTAL DE LA VENTA')
        ...
        ->setCellValue('N' . $punt, 'GLOSA FACTURA');

// $this->phpexcel->getFont()->setBold(false);

$this->phpexcel->getActiveSheet()->getStyle('A' . $punt . ':N' . $punt)->applyFromArray(
        array(
            'font' => array(
                'bold' => true,
                'size' => 9,
...
foreach ($registros->result() as $reg) {
    ...
    if ($cont_fac[$reg->id_factura]->num_rows() > 0)
        ...
        $c = "Sin contrato";
    if ($glos_fac[$reg->id_factura] != "")
        $glo = $glos_fac[$reg->id_factura];
    else
        $glo = "-";
    ...
    ->setCellValue('B' . $punt, $fechha)
    ->setCellValue('C' . $punt, $reg->num_factura)
    ->setCellValue('D' . $punt, " " . $reg->nro_autorizacion)
    ->setCellValue('E' . $punt, $reg->est_factura)
    ->setCellValue('F' . $punt, " " . $reg->NIT_cliente)
    ->setCellValue('G' . $punt, $reg->razon_social)
    ->setCellValue('H' . $punt, $reg->monto_total_bs)
    ->setCellValue('I' . $punt, '=H' . $punt . '*0.13')
    ->setCellValue('J' . $punt, $reg->codigo_control)
    ->setCellValue('K' . $punt, $p)
    ->setCellValue('L' . $punt, $c)
    ->setCellValue('M' . $punt, $reg->comentario)
    ->setCellValue('N' . $punt, $glo);
     ...
     */
    $this->phpexcel->getActiveSheet()->getStyle('A' . $punt . ':N' . $punt)->applyFromArray($style_ok);
    if ($reg->est_factura != 'Valido') {
        $this->phpexcel->getActiveSheet()->getStyle('A' . $punt . ':N' . $punt)->applyFromArray($style_no);
        $this->phpexcel->setActiveSheetIndex(0)
                ->setCellValue('H' . $punt, '0')
                ->setCellValue('I' . $punt, '=H' . $punt . '*0.13')
                ->setCellValue('J' . $punt, '');
    }
    $this->phpexcel->getActiveSheet()->getStyle('L' . $punt . ':N' . $punt)->getAlignment()->setWrapText(true);
    $punt++;
}
...
$this->phpexcel->getActiveSheet()->getColumnDimension('n')->setWidth(85);

$this->phpexcel->getActiveSheet()->setTitle('Facturas');
...
header('Content-Disposition: attachment;filename="Reporte_facturacion.xlsx"');
