...
///cabecera
    ...
    $this->pdf->Ln(5);
    $this->pdf->Cell(120, 4, ' ', '', 0, 'R', '0');
    $this->pdf->Cell(35, 5, utf8_decode('NOTA FISCAL N°:'), 'L', 0, 'R', '1');
    $this->pdf->SetFont('Times', 'B', 12);
    $this->pdf->SetTextColor(141, 0, 40);
    $can_le = strlen($dato_factura->num_factura . "");
    $cero = substr("0000", $can_le);
    $this->pdf->Cell(35, 5, $cero . $dato_factura->num_factura, 'R', 0, 'L', '0');
    $this->pdf->SetFont('Arial', 'B', 10);
    $this->pdf->SetTextColor(0, 0, 0);
    $this->pdf->Ln(5);
    $this->pdf->Cell(120, 4, ' ', '', 0, 'R', '0');
...
//->MultiCell($w, $alto, $data[$i], 0, $a, false);
    ...
    $this->pdf->Cell(70, 5, utf8_decode("NOTA DE"), '', 0, 'C', '0');
    $this->pdf->Ln(3);
    $yyy = $this->pdf->getY();
    $this->pdf->setY(35);
    $this->pdf->Cell(60, 4, '', '', 0, 'C', '0');
    $this->pdf->Cell(70, 5, utf8_decode("CRÉDITO - DÉBITO"), '', 0, 'C', '0');
    $this->pdf->setY($yyy);
    $this->pdf->SetFont('Arial', '', 6);
...
//*********************************************************
    ...
    $this->pdf->SetFont('Arial', 'B', 10);
    $this->pdf->Cell(110, 5, utf8_decode($dato_factura->razon_social), '', 0, 'L', '0');
    $this->pdf->SetFont('Arial', '', 10);
    $this->pdf->Cell(15, 5, 'NIT/CI:', '', 0, 'L', '0');
    $this->pdf->SetFont('Arial', 'B', 10);
    $this->pdf->Cell(35, 5, $dato_factura->NIT_cliente, '', 0, 'L', '0');
    $this->pdf->SetFont('Arial', '', 10);
    $this->pdf->Ln(7);
//$this->pdf->Cell(15, 4,' ' , '', 0, 'R', '0');
    $this->pdf->SetLineWidth(.5);
    $this->pdf->SetFillColor(210, 230, 246);
    $this->pdf->SetFont('Arial', 'B', 12);
    $this->pdf->Cell(190, 7, utf8_decode('DATOS DE LA TRANSACCIÓN ORIGINAL'), 'T', 0, 'C', '1');
    $this->pdf->Ln(6);
    $this->pdf->SetFont('Arial', '', 11);
    $this->pdf->Cell(63, 5, '2', '', 0, 'C', '1');
    $this->pdf->Cell(64, 5, '222', '', 0, 'C', '1');
    $this->pdf->Cell(63, 5, '5555', '', 0, 'C', '1');
    $this->pdf->Ln(4);
    $this->pdf->SetFont('Arial', 'B', 8);
    $this->pdf->SetLineWidth(.2);
    $this->pdf->Cell(63, 5, utf8_decode('Número Factura'), 'B', 0, 'C', '1');
    $this->pdf->Cell(64, 5, utf8_decode('Número Autorización'), 'B', 0, 'C', '1');
    $this->pdf->Cell(63, 5, utf8_decode('Fecha de Emisión'), 'B', 0, 'C', '1');
    $this->pdf->Ln(4);
    ...
    $this->pdf->Ln(4);
    foreach ($detalle_factura_org->result() as $reg) {
    ...
    $this->pdf->Ln(15);
    $this->pdf->SetFillColor(210, 230, 246);
    $this->pdf->Cell(190, 7, utf8_decode('DETALLE DE LA DEVOLUCIÓN O RESCISIÓN DE SERVICIO'), 'T', 0, 'C', '1');
    $this->pdf->Ln(6);
    ...
    $this->pdf->Ln(4);
    foreach ($detalle_factura_dev->result() as $reg) {
        $datos = array(number_format($reg->cantidad, 2, '.', ','), utf8_decode($reg->detalle_ps), number_format($reg->precio, 2, '.', ','), number_format($reg->importe, 2, '.', ','));
        $this->pdf->RowBodyFactura($datos, 4);
    }
    ...
    $this->pdf->Cell(135, 6, '', 'T', 0, 'R', '1');
    $this->pdf->Cell(25, 6, ' TOTAL Bs.-', 'T', 0, 'R', '1');
    $this->pdf->Cell(30, 6, number_format($dato_factura->monto_devolucion, 2, '.', ','), 'T', 0, 'R', '1');
    $this->pdf->Ln(5);
...
//$this->pdf->Ln(3);
    ...
    $this->pdf->Cell(160, 10, utf8_decode('Monto Efectivo del Crédito o Débito (13% del Importe Total Devuelto)'), 'TBL', 0, 'L', '1');
    $this->pdf->SetFont('Arial', 'B', 10);
    $this->pdf->Cell(30, 10, number_format(($dato_factura->monto_devolucion * 0.13), 2, '.', ','), 'TBR', 0, 'R', '1');
    $this->pdf->Ln(10);
    ...
    $this->pdf->Image(base_url() . 'imagenesweb/nota_fiscal_QR/' . $dato_factura->codigo_qr_name . '.png', null, null, 27, 'jpg');
    $this->pdf->Ln(-23);
