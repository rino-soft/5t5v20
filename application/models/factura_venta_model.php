 ...
 */
class factura_venta_model extends CI_Model {
    ...
    function actualiza_qr_ruta($id_factura, $name, $cc) {
    ...
    function listar_buscar_factura_venta_cantidad($busqueda, $de, $ha) {
        $busqueda = str_replace(" ", "%", $busqueda);
        $sql = "select * from factura_venta fv ,cliente C,usuarios U 
    ...
    function listar_buscar_factura_venta($busqueda, $ini, $cant, $de, $ha) {
        $busqueda = str_replace(" ", "%", $busqueda);
        ...
        $sql = "select *,fv.fh_registro as fec_reg , fv.estado as est_factura from factura_venta fv ,cliente C,usuarios U 
            ...
            and concat(fv.id_factura,fv.monto_total_bs,fv.comentario,fv.id_contrato,C.razon_social,C.nit) lIKE '%$busqueda%'
             ...
             order by fv.id_factura DESC limit $ini,$cant";
        $consulta = $this->db->query($sql);
        return($consulta);
    }
...
//if (strlen($num) > 14) die("El n?mero introducido es demasiado grande"); 
    ...
    function obtener_factura_venta($id_factura) {
        $sql = "select * from factura_venta fv where fv.id_factura=$id_factura";
        $consulta = $this->db->query($sql);
        return($consulta->row());
    }
    ...
    function obtener_detalle_factura_venta($id_factura) {
        $sql = "select * from factura_venta_detalle fvd where fvd.id_factura=$id_factura";
        $consulta = $this->db->query($sql);
        return($consulta);
    }
    function obtener_detalle_factura_venta_producto($id_factura) {
        $sql = "select * from factura_venta_detalle fvd , producto_servicio ps where fvd.id_factura=$id_factura";
        $consulta = $this->db->query($sql);
        return($consulta);
    }
