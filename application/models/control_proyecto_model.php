 ...
 */
class control_proyecto_model extends CI_Model {
    ...
    function listar_sitios_x_proyecto($id_proy) {
        ...
        if ($id_proy == 0) {
            ...
            foreach ($proyectos->result() as $p) {
                ...
                $sw = 1;
                $codigo.=" st.id_proyecto=" . $p->id_proy;
            }
        } else
            $codigo = "st.id_proyecto=" . $id_proy;

        $sql = "select st.*,p.nombre as nompreproy FROM sitiotrabajo st left join proyecto p on st.id_proyecto=p.id_proy where $codigo";
        $consulta = $this->db->query($sql);
        return($consulta);
    }
    ...
    function save_sitio() {
        ...
        if ($this->input->post('idsitio') == 0) {
            ...
            $respuesta = "<input type='hidden' id='ayudata' value='$id_insert'><input type='hidden' id='ayudatamensaje' value='El Registro se ha Registrado Exitosamente !!'>";
        } else {
    ...
    function obtener_monto_sitio($id_sitio) {
        $sql = "SELECT idDUID,sum(monto)as montoDIUD FROM bd_erp_sg.itemsitiomonto where idDUID=$id_sitio group by idDUID;";
        $consulta = $this->db->query($sql);
        $resultado = 0;
    ...
    function obtener_monto_sitio_mes($id_sitio, $mes, $proyecto) {
        $codigo_proy = "";
        $codigo_sitio = "";
        $fa = strtotime($mes);
        ...
        if ($proyecto == 0) { //codigo para aumentar los proyecto;
            ...
            foreach ($proyectos->result() as $p) {
                ...
                $sw = 1;
                $codigo_proy.=" st.id_proyecto=" . $p->id_proy;
            }
        } else
            $codigo_proy = "st.id_proyecto=" . $proyecto;
        if ($id_sitio != 0) {
            //codigo para aumentar el sitio
            $codigo_sitio = "and st.idSitioTrabajo=" . $id_sitio;
        }


        $sql = "select ifnull(sum(ism.monto),0) as total from itemsitiomonto ism left join sitiotrabajo st on ism.idDUID=st.idSitioTrabajo
                where 
                 (" . $codigo_proy . ")
                 " . $codigo_sitio . "    
                and  month(ism.fecha_correspondiente) =" . date("m", $fa) . " and year(ism.fecha_correspondiente)=" . date("Y", $fa);
    ...
    function delete_item_po_sitio() {
        $this->db->where('iditemSitiomonto', $this->input->post("item"));
        $this->db->delete('itemSitiomonto');
        return("El registro fue Eliminado");
    }

    function save_item_po_sitio() {
        ...
        $id_det = explode("|", $this->input->post("id_eetalle"));
        $npos = explode("|", $this->input->post("npo"));
        $desc = explode("*|*", $this->input->post("desc_item"));
        $mont = explode("|", $this->input->post("monto_desc"));
        $id_us = explode("|", $this->input->post("usuario_asignado"));
        $respuesta = "";
        ...
        for ($i = 0; $i < count($desc) - 1; $i++) {
            $datos = array(
                ...
                "monto" => $mont[$i],
                'fecha_creacion' => date("Y-m-d H:i:s"),
                'fecha_correspondiente' => date($this->input->post("mes_corresp") . "-01"),
                "user_asignado" => $id_us[$i]
            );
            if ($id_det[$i] == 0) {
                $this->db->insert('itemSitiomonto', $datos);
                $id_insert_detalle = ($this->db->insert_id());
                $respuesta.="* registro :$id_insert_detalle |  PO:$npos[$i] | Nuevo Registro";
            } else {
                //echo "nuevos";
                $this->db->where('iditemSitiomonto', $id_det[$i]);
                $upd = $this->db->update('itemSitiomonto', $datos);
                $respuesta.="*registro :$id_insert_detalle |  PO:$npos[$i] | Registro Editado";
            }
        }
    ...
    function listar_buscar_orden_comprasxsitio($idsitio) {
        $sql = "select * FROM itemSitiomonto where idDUID=$idsitio";
        $consulta = $this->db->query($sql);
        return($consulta);
    }

    function obtener_datos_item_po($id_po) {
        $sql = "select * FROM itemSitiomonto where iditemSitiomonto=$id_po";
        $consulta = $this->db->query($sql);
        return($consulta);
    }

    function obtener_datos_po($id_po) {
        $sql = "select * FROM ordencompra where idordenCompra=$id_po";
        $consulta = $this->db->query($sql);
        return($consulta);
    }
