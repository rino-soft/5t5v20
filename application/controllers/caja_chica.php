...
//$padre,$hijo
    function index($padre, $proyecto = 0) {
        echo "funciona";
        $data['titulo'] = 'Solicitud de Cajas Chica';
        ...
        $data['registros'] = $this->caja_chica_model->listar_buscar_caja_chica($proyecto);
        $data['registros2'] = $this->control_proyecto_model->listar_sitios();
        ...
        $sumaUtilidad = 0;
        $proy_montos = Array();
//        foreach ($data['registros']->result() as $solpago) {
//            $proy_montos[$solpago->id_sol_frendir] = $this->fondosRendir_model->obtener_datos_proyecto_monto($solpago->id_sol_frendir, $proyecto);
//        }//obtiene montos    
        foreach ($data['registros2']->result() as $sitio) {
        ...
        $data['vista_enviada'] = 'caja_chica/caja_chica_view';
        $this->load->view('Plantilla/Plantilla_vista', $data);
    }

    function nuevo_cc($id_cc) {
        ...
        $data['id_rend'] = $id_cc; //estoy cambiando id_ov_f
        $data['sitios'] = "Selecciona un proyecto";
        $detalle = array();
        if ($id_cc != 0) {

//
            $data['datos_solicitud'] = $this->caja_chica_model->obtener_datos_cajc_chica($id_cc);
            //$data['detalle_solicitud'] = $this->caja_chica_model->obtener_datos_sol_fondos_detalle($id_rendicion);
...
//            $detalle['datos_rendicion_detalle_sgrC'] = $this->rendiciones_model->obtener_detalle_datos_rendicion($id_rendicion, 'tel', 1);
        ...
        $this->load->view('caja_chica/form_new_caja_chica_view', $data);
    }

    function obtener_cajas_chicas_usuario() {
        $user = $this->input->post("id_user");
        $res = $this->caja_chica_model->obtener_cc_user($user);
        $ccs = "";
        if ($res->num_rows() > 0) {
            foreach ($res->result() as $cc) {
                $ccs.= "<tr><td>$cc->idcaja_chica</td><td>$cc->nombre</td><td>$cc->monto</td><td>$cc->estado</td></tr>";
            }
            echo "<div class='bg-info'><span class'black'>Información .-</span> el usuario cuenta con otras cajas chicas"
            . "<table class='table'><tr><th>ID Caja Chica</th><th>Proyecto</th><th>Monto</th><th>Estado</th></tr>$ccs</table></div>";
        }
    }

    function guardar_nueva_cc() {
        echo $this->caja_chica_model->save_scc();
    }
